...
{
    class TradeCancelledRepository : RepositoryBase
    {
        ...
        public int InsertTrades(List<Trade_Cancelled> trades, Int32 batchNos = 100)
        {
            string ssql = string.Format("delete Trade_Cancelled");
            context.Database.ExecuteSqlCommand(ssql);
            return InsertEntities<Trade_Cancelled>(context, trades);
        }

        #region extract trades from Gloss popultate trade entities

        public DataSet ExtractTradesFromTE()
        {
            ...
            {
                string filters = string.Format(" WHERE T.trade_type_wil IN ('{0}') AND T.trade_status_p2k IN ('X','C') {1} AND T.version_date > DATEADD(month, -1, GETDATE())", // Past One Month
        ...
        public List<Trade_Cancelled> GetTradeListFromTE(DateTime Run_Date)
        {
            var ds = ExtractTradesFromTE();
            var dvContra = new DataView(ds.Tables["Contra_Refs"]);

            var Trades = new Dictionary<Int64, Trade_Cancelled>();
            ...
            foreach (DataRow row in ds.Tables[0].Rows)
            {
                ...
                //bool is_Contra = (tradeType == TradeType.FREC);
        ...
        private static void PopulateExecSalesPerson(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void PopulateClientDepotAccounts(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void PopulateCompanyDepotAccounts(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void PopulateTradeExtRefs(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void PopulateTradeDrivers(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void PopulateTradeEvent(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static Trade_Cancelled PopulateTradeInfo(DateTime BusinessDate, DataRow row, bool Is_Contra)
        {
            var trade = new Trade_Cancelled()
            {
        ...
        private static void PopulateSplitStatus(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void PopulateLinkStatus(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void SetAmountByTradeStatus(DataRow row, Trade_Cancelled trade)
        {
        ...
        private static void PopulateContraRef(DataView dvContra , Trade_Cancelled trade, Dictionary<Int64, Trade_Cancelled> trades)
        {   
